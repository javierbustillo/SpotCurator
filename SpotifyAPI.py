...
class SpotifyAPI:
    ...
    def get_liked_songs(self, token):
        return self.request_data('/me/tracks?limit=50', token=token)

    def get_playlist_tracks(self, token, pid):
        return self.request_data('/playlists/%s/tracks' % pid, token=token)

    def get_features(self, token, tid):
        return self.request_data('/audio-features/%s' % tid, token=token)